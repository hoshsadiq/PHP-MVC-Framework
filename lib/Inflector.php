...
class Inflector
{
    ...
    public static function pluralize($word)
    {
        ...
        if (!isset(self::$_plural['merged']['uninflected'])) {
            self::$_plural['merged']['uninflected'] = array_merge(
                self::$_plural['uninflected'],
                self::$_uninflected
            );
        }
        ...
        if (preg_match(
            '/(.*)\\b(' . self::$_plural['cacheIrregular'] . ')$/i',
            $word,
            $regs
        )
        ) {
            self::$_cache['pluralize'][$word] = $regs[1] . substr($word, 0, 1) . substr(
                    self::
                        $_plural['merged']['irregular'][strtolower($regs[2])],
                    1
                );
            return self::$_cache['pluralize'][$word];
        }
    ...
    public static function singularize($word)
    {
        ...
        if (preg_match(
            '/(.*)\\b(' . self::$_singular['cacheIrregular'] . ')$/i',
            $word,
            $regs
        )
        ) {
            self::$_cache['singularize'][$word] = $regs[1] . substr($word, 0, 1) . substr(
                    self::
                        $_singular['merged']['irregular'][strtolower($regs[2])],
                    1
                );
            return self::$_cache['singularize'][$word];
        }
    ...
    /**
     * Checks whether to return a plural or singular form
     *
     * @param int $count The number to check
     * @param string $word The word to pluralize/singularize
     */
    public static function coutable($count, $word)
    {
        return $count . ' ' . (((int)$count) == 1 ? self::singularize($word) : self::pluralize($word));
    }
}
